...
/**
 * ChatModal — Chat en tiempo real entre cliente y conductor durante el servicio.
 *
 * Props:
 *  isOpen        {boolean}  — si el modal está visible
 *  onClose       {function} — cerrar el modal
 *  requestId     {string}   — ID de la solicitud activa
 *  currentUserId {string}   — ID del usuario actual
 *  otherName     {string}   — Nombre de la contraparte (conductor o cliente)
 */
const ChatModal = ({ isOpen, onClose, requestId, currentUserId, otherName }) => {
  ...
  useEffect(() => {
    ...
    const handleIncoming = (msg) => {
      ...
      setMessages((prev) => {
        // Evitar duplicados (el emisor también recibe confirmación)
      ...
      <IonHeader>
        <IonToolbar>
          <IonTitle className="chat-modal-title">
            <span className="chat-modal-title-icon">💬</span> {otherName || 'Chat'}
          </IonTitle>
          <IonButtons slot="end">
